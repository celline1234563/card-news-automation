...
// ── Step 1: 톡톡 3-2 페이지 찾기 ──
console.log('═══ Step 1: 톡톡 3-2 페이지 찾기 ═══');

const searchStatuses = ['기획 컨펌', '제작 요청', '디자인 1차', '기획컨펌대기', '원고작업'];
let targetPage = null;
...
if (!targetPage) {
  console.error('톡톡 3-2 페이지를 찾을 수 없습니다.');
...
try {
  ...
  // 오늘 날짜 폴더에 없으면, glob으로 이전 폴더 검색
  console.log('  오늘 날짜 폴더에 copy.json 없음 — 이전 폴더 검색');
  try {
    const matches = await glob('output/*톡톡*3-2*/copy.json', { cwd: ROOT });
    if (matches.length === 0) {
      // 톡톡 3-2 와 관련된 패턴으로 더 넓게 검색
      const matches2 = await glob('output/*톡톡*32*/copy.json', { cwd: ROOT });
...
// copy.json을 아직 못 찾았으면 리서치 + 기획 재생성
if (!copyData) {
  ...
  const topic = targetPage.title.replace(/^\[.*?\]\s*/, '') || '톡톡 3-2';
