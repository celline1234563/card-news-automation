/**
 * 톡톡 잉글리쉬 기존 copy.json을 수정된 템플릿으로 재적용 + 재렌더링
 *
 * 사용법: node scripts/retemplate-toktok.js [폴더명]
 *   폴더명 생략 시 모든 톡톡 잉글리쉬 폴더 대상
 */
...
async function main() {
  ...
  if (!academy) {
    console.error('톡톡 잉글리쉬 설정을 찾을 수 없습니다.');
    process.exit(1);
  }
  ...
  if (targetFolder) {
    ...
    folders = allDirs.filter(d => d.startsWith('톡톡 잉글리쉬'));
  }
