...
const OUTPUT_DIR = join(ROOT, 'output', '올인원 수학학원-과제-이행률-100--재디자인-2026-03-19');

// ── Step 1: 노션에서 중등3-3 페이지 찾기 ──
console.log('═══ Step 1: 노션에서 올인원 중등3-3 페이지 찾기 ═══');
...
for (const status of searchStatuses) {
  ...
  try {
    ...
    const found = pages.find(p =>
      ...
      (p.title.includes('중등3-3') || p.title.includes('중등 3-3'))
    );
    if (found) {
      targetPage = found;
...
if (!targetPage) {
  console.error('올인원 중등3-3 페이지를 찾을 수 없습니다.');
  process.exit(1);
}
